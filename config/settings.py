...
REGISTRATION_MODE = 'drag'            # Режим захвата снимков:
                                      ...
                                      #   'background'     — простое вычитание фона (absdiff)
                                      #   'background_mog2' — адаптивное вычитание фона (MOG2, для разноцветных фонов)

# Настройки MOG2 (адаптивное фоновое вычитание)
MOG2_BG_FRAMES = 15                   # Кадров для обучения модели на чистом фоне
MOG2_VAR_THRESHOLD = 16               # Порог дисперсии; выше → меньше шума, но может пропустить медленные движения
MOG2_DETECT_SHADOWS = False           # Игнорировать тени (быстрее, проще)
MOG2_MIN_AREA = 50                    # Минимальная площадь (пиксели) для ROI; отфильтровать шум
MOG2_MAX_BLOB_RATIO = 0.75            # Максимальный размер blob как доля размера кадра;
                                      # blob крупнее → весь кадр засчитан foreground → повтор снимка

# Temporal Stability Filter (фильтр временной стабильности SIFT-признаков)
# Оставляет только те точки, которые устойчиво появляются в одном месте N кадров подряд.
# Подавляет точки анимированного/меняющегося фона до этапа FLANN-сопоставления.
# НЕ интегрирован в pipeline — изменение этого флага ничего не включает автоматически.
# Для активации: инстанцировать TemporalStabilityFilter из scanner/temporal_filter.py
# и вызывать .filter(kp, des) после extract_features() в tracking_loop.
TEMPORAL_FILTER_ENABLED   = False  # не подключён; флаг только для документирования
TEMPORAL_FILTER_FRAMES    = 3      # размер окна истории (кадров)
TEMPORAL_FILTER_MIN_HITS  = 2      # минимум кадров, в которых точка должна появиться
TEMPORAL_FILTER_CELL_SIZE = 8      # размер ячейки сетки в пикселях (квантование позиций)
...
# Настройки трекера (APPEARED/DISAPPEARED и сглаживание позиции)
TRACKER_MAX_HISTORY = 10       # сколько последних позиций хранить для сглаживания bbox
TRACKER_INACTIVE_FRAMES = 8    # кадров без детекции до DISAPPEARED; меньше → быстрее сброс, больше → меньше мигания
