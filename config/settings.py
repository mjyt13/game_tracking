...
# Язык веб-интерфейса ('ru' или 'en')
LANGUAGE = 'ru'

# Сетевые настройки HTTP-сервера
HTTP_PORT = 5000
MDNS_ENABLED = True       # регистрировать имя <MDNS_HOSTNAME>.local (требует пакет zeroconf)
MDNS_HOSTNAME = 'game'    # адрес для устройств: http://game.local:5000/

# Пароль доступа к панели ведущего (/) — простая авторизация для демонстрации.
# ВАЖНО: смените перед реальным использованием. Это не криптостойкая защита.
ADMIN_PASSWORD = 'admin'

# Игровые настройки
ALLOW_SINGLE_PLAYER = False  # False → старт игры требует 2+ игроков (иначе сигнал в вебе и консоли)
...
CV2_SHOW_OVERLAY = False  # Показывать в окне cv2 панель команд и кнопки управления
                         # (сетка grid управляется отдельно клавишей [G])
CV2_SHOW_BBOX = True     # Показывать рамку (bbox) и метку вокруг обнаруженных фишек в окне cv2

# Заливка клеток с эффектами и нумерация клеток.
# Рисуются только на отображаемом кадре (detection не затрагивается) и видны
# вместе с сеткой — переключаются той же клавишей [G].
CELL_FILL_ALPHA = 0.35   # прозрачность заливки (0 = невидима, 1 = непрозрачна)
# Цвет заливки по эффекту клетки, формат BGR (как в OpenCV).
# Ключи совпадают с константами эффектов из game/events.py + 'FINISH' для финишной клетки.
CELL_EFFECT_COLORS = {
    'SKIP_TURN':    (60, 60, 220),    # красный    — пропуск хода
    'MOVE_BACK':    (50, 140, 240),   # оранжевый  — назад
    'MOVE_FORWARD': (80, 200, 80),    # зелёный    — вперёд
    'EXTRA_TURN':   (210, 180, 60),   # бирюзовый  — доп. ход
    'FINISH':       (60, 215, 240),   # золотой    — финиш
}
# Альтернативная палитра без красно-зелёной пары (для дальтоников) — при необходимости
# скопировать в CELL_EFFECT_COLORS:
#   'SKIP_TURN': (40,90,230), 'MOVE_BACK': (0,150,255), 'MOVE_FORWARD': (200,140,0),
#   'EXTRA_TURN': (180,180,180), 'FINISH': (60,215,240)
CELL_NUMBER_COLOR = (255, 255, 255)  # цвет номеров клеток (BGR), с тёмной обводкой
# Положение номера внутри клетки: 'center', 'tl' (сверху-слева), 'tr' (сверху-справа),
# 'bl' (снизу-слева), 'br' (снизу-справа). В углу номер меньше загораживает фишку.
CELL_NUMBER_POSITION = 'tl'
